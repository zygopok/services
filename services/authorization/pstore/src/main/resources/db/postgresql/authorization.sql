alter table permissions_actions drop constraint FK85F8204226A2CC74;
drop table accounts_roles;
drop table permissions;
drop table permissions_actions;
...
create table roles (csid varchar(128) not null, created_at timestamp not null, description varchar(255), displayname varchar(200) not null, metadata_protection varchar(255), perms_protection varchar(255), rolegroup varchar(255), rolename varchar(200) not null, tenant_id varchar(128) not null, updated_at timestamp, primary key (csid), unique (rolename, tenant_id), unique (displayname, tenant_id));
alter table permissions_actions add constraint FK85F8204226A2CC74 foreign key (ACTION__PERMISSION_CSID) references permissions;
